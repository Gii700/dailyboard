...
import { muatDariStorage, muatCatatanDariStorage, simpanKeStorage } from "./ntut/storege.js";
import { ambilCuaca, ambilKutipan } from "./ntut/api.js";
import { validasiInput, tambahTugas, hapusTugas, toggleSelesai } from "./ntut/tugas.js";
import { tambahCatatan, hapusCatatan } from "./ntut/catatan.js";
...
function renderTugas(filter = filterAktif) {
  ...
  tugasTersaring.forEach((tugas, index) => {
    ...
    // MINGGU 6: Klik 1x untuk Tandai Selesai (Disinkronkan dengan tugas.js)
    ...
    // MINGGU 5: Tombol Hapus Tugas (Disinkronkan dengan tugas.js)
...
// =========================================================================
// EVENT LISTENERS MANAGEMENT (Hanya Satu Pengatur per Komponen)
// =========================================================================

// Event Listener Utama untuk Tombol Tambah Tugas
tombolTambah.addEventListener("click", () => {
  const teks = inputTugas.value.trim(); 

  if (validasiInput(teks)) { 
    // Memanggil fungsi modular dengan 3 parameter
    const hasil = tambahTugas(daftarTugas, teks, nextId);
...
tombolTambahCatatan.addEventListener("click", () => {
  ...
  if (validasiInput(isi)) {
    const hasilCatatan = tambahCatatan(daftarCatatan, isi);
    daftarCatatan = hasilCatatan.daftarBaru || hasilCatatan;
    inputCatatan.value = "";
    renderCatatan();
  }
});
...
async function muatSemuaWidget() {
  ...
  try {
    ...
    statusWidget.textContent = "Gagal memuat beberapa data widget.";
  }
}
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  // 2. Muat Data dari Storage ke Variabel State Global
  ...
  // 4. MINGGU 12: Panggil API Widget Pihak Ketiga
  muatSemuaWidget();
});
